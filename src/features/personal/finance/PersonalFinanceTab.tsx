...
export function PersonalFinanceTab() {
  ...
  const sorted = useMemo(() => sortCycles(personalFinanceCycles as Cycle[]), [personalFinanceCycles])
    ...
    <div className="flex-1 overflow-y-auto">
      <div className="px-6 pt-5 pb-3 flex items-end justify-between">
        ...
        <button
          ...
          style={{ borderColor: 'rgba(34,211,238,0.3)', color: '#22d3ee', backgroundColor: 'rgba(34,211,238,0.1)' }}
        >
      ...
      <div className="px-4 pb-8 space-y-3">
        {sorted.length === 0 ? (
          <div className="text-center py-16">
            <p className="text-sm" style={{ color: 'rgba(255,255,255,0.25)' }}>No finance tasks yet</p>
          </div>
        ) : (
      ...
      {adding && (
        <PersonalQuickAdd area="personal-finance" onClose={() => setAdding(false)} />
      )}
    </div>
  )
}
