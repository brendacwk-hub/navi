...
export function ToBuyTab() {
  ...
  const sorted = useMemo(() => sortCycles(tobuyCycles as Cycle[]), [tobuyCycles])

  return (
    <div className="flex-1 overflow-y-auto">
      <div className="px-6 pt-5 pb-3 flex items-end justify-between">
        <div>
          <h2 className="text-lg font-semibold text-white">To Buy</h2>
        ...
        <button
          ...
          style={{ borderColor: 'rgba(252,211,77,0.3)', color: '#fcd34d', backgroundColor: 'rgba(252,211,77,0.1)' }}
        >
      ...
      <div className="px-4 pb-8 space-y-3">
        {sorted.length === 0 ? (
          <div className="text-center py-16">
            <p className="text-sm" style={{ color: 'rgba(255,255,255,0.25)' }}>Nothing to buy yet</p>
            <p className="text-xs mt-1" style={{ color: 'rgba(255,255,255,0.15)' }}>Tap Add to create a shopping list</p>
          </div>
        ) : (
      ...
      {adding && (
        <PersonalQuickAdd area="tobuy" onClose={() => setAdding(false)} />
      )}
    </div>
  )
}
