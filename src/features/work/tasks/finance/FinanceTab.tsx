...
const FINANCE_SUB_AREAS = ['Payments', 'Budgets', 'Administrative', 'Records', 'AI']
...
function groupBySubArea(cycles: Cycle[], subAreaOrder: string[]): { subArea: string; cycles: Cycle[] }[] {
  ...
  for (const [sub, items] of groups) {
    if (!FINANCE_SUB_AREAS.includes(sub)) result.push({ subArea: sub, cycles: items })
  }
...
function FinanceTabInner() {
  ...
  const setSubArea = (sub: string | null) => {
    ...
    router.push(sub ? `/work/finance?sub=${encodeURIComponent(sub)}` : '/work/finance')
  }

  // Sub-area tabs ordered by closest deadline
  const sortedSubAreas = useMemo(() => {
    const minDate = new Map<string, number>()
    for (const sub of FINANCE_SUB_AREAS) {
      const min = financeCycles
        .filter(c => c.subArea === sub)
        .reduce((m, c) => Math.min(m, computeSortDate(c.triggerLabel)), Infinity)
      minDate.set(sub, min)
    }
    return [...FINANCE_SUB_AREAS].sort((a, b) => {
      const da = minDate.get(a) ?? Infinity
      const db = minDate.get(b) ?? Infinity
      if (da === db) return FINANCE_SUB_AREAS.indexOf(a) - FINANCE_SUB_AREAS.indexOf(b)
      return da - db
    })
  }, [financeCycles])

  const countBySub = useMemo(() => FINANCE_SUB_AREAS.reduce<Record<string, number>>((acc, sub) => {
    acc[sub] = financeCycles.filter(c => c.subArea === sub).length
    return acc
  }, {}), [financeCycles])
  ...
  const flatItems = useMemo(() => {
    ...
    return extractFlatItems(financeCycles.filter(c => matchesCycle(c, query)))
  }, [chipFilter, financeCycles, query])
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      ...
      <div className="px-6 pt-5 pb-3">
        ...
        <p className="text-xs text-white/35 mt-0.5">
          {chipFilter === 'Latest'
            ...
            : sortedFiltered.length < financeCycles.length || query
              ? <>{sortedFiltered.length} of {financeCycles.length} cycles{query && <span className="ml-1 text-finance/70">— &quot;{query}&quot;</span>}</>
              : <>{financeCycles.length} cycles</>}
        </p>
      </div>
      ...
      <div className="relative border-b border-white/8">
        <div className="flex overflow-x-auto overflow-y-hidden scrollbar-none px-3">
          <button
            ...
            className={`flex-shrink-0 px-4 py-2.5 text-[13px] font-medium border-b-2 -mb-px whitespace-nowrap transition-all ${
              !activeSub ? 'border-finance text-white' : 'border-transparent text-white/40 hover:text-white/70'
            }`}
          >
            All
          </button>
          {sortedSubAreas.map(sub => (
            <button
              ...
              className={`flex-shrink-0 px-4 py-2.5 text-[13px] font-medium border-b-2 -mb-px whitespace-nowrap transition-all ${
                activeSub === sub ? 'border-finance text-white' : 'border-transparent text-white/40 hover:text-white/70'
              }`}
            >
              {sub}
              <span className={`ml-1 text-[11px] ${activeSub === sub ? 'text-finance/60' : 'text-white/20'}`}>
                ({countBySub[sub]})
              </span>
            </button>
          ))}
        ...
        <div className="flex absolute right-0 top-0 bottom-0 items-stretch pointer-events-none">
          <div className="w-8 bg-gradient-to-l from-[rgba(12,12,12,1)] to-transparent" />
        </div>
        <div className="flex absolute right-0 top-0 bottom-0 items-center">
          <button
          ...
          <>
            ...
            <div className="absolute right-0 top-full mt-1 z-50 bg-sidebar border border-white/10 rounded-xl shadow-2xl py-1.5 w-[200px]">
              <div className="px-3 pb-1.5 mb-0.5 border-b border-white/6">
                <span className="text-[10px] font-bold text-finance/60 uppercase tracking-widest">Finance</span>
              </div>
              <button onClick={() => setSubArea(null)}
                className={`w-full text-left px-4 py-2 text-[13px] transition-colors ${
                  !activeSub ? 'text-finance font-semibold' : 'text-white/60 hover:text-white hover:bg-white/5'
                }`}
              >
                All
              </button>
              {sortedSubAreas.map(sub => (
                <button key={sub} onClick={() => setSubArea(sub)}
                  className={`w-full text-left px-4 py-2 text-[13px] flex items-center justify-between transition-colors ${
                    activeSub === sub ? 'text-finance font-semibold' : 'text-white/60 hover:text-white hover:bg-white/5'
                  }`}
                >
                  <span>{sub}</span>
          ...
          flatItems.length === 0 ? (
            ...
            <div className="space-y-0.5">
              {flatItems.map(item => (
                ...
                >
                  <button
                    onClick={() => toggleItem('finance', item.cycleId, item.id)}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                      ...
                      {item.subArea && (
                        <span className="text-[10px] px-1.5 py-0.5 rounded bg-finance/10 text-finance/60 border border-finance/15">
                          {item.subArea}
                        </span>
                      )}
                    </div>
              ...
              <div key={subArea || '_none'}>
                {subArea && (
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-[11px] font-semibold uppercase tracking-widest text-finance/60">{subArea}</span>
                    <div className="flex-1 h-px bg-finance/15" />
                  </div>
                )}
