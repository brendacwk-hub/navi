          ...
          <div className="rounded-xl border border-navi-blue/25 bg-navi-blue/8 p-4">
            ...
            <div className="grid grid-cols-3 gap-1.5 sm:flex sm:flex-wrap sm:items-center">
              {chainItems.map((step, i) => {
                ...
                return (
                  <div key={step.id} className="contents sm:flex sm:items-center sm:gap-1.5">
                    <button
                      onClick={() => cycle && setSheetCycle(cycle)}
                      disabled={!cycle}
                      className={`text-[11px] px-2 py-1 rounded-md font-medium transition-all active:scale-95 w-full sm:w-auto text-left truncate ${
                    ...
                    {i < chainItems.length - 1 && (
                      <ChevronRight className="hidden sm:block w-3 h-3 text-white/15 flex-shrink-0" />
                    )}
                  </div>
                )
              })}
