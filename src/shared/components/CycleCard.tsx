            ...
            <div className="space-y-2" onClick={e => e.stopPropagation()}>
              ...
              <div className="flex gap-1.5 flex-wrap items-center">
                {(['Today', 'Tomorrow', 'In 2 Days'] as const).map(d => (
                  <button key={d} onMouseDown={ev => ev.preventDefault()} onClick={() => { setEditDue(resolveLabel(d)); setEditRecurr('') }}
                    className={`text-[10px] px-2 py-0.5 rounded border transition-all ${
                      !editRecurr && editDue === resolveLabel(d) ? 'bg-navi-blue/20 text-navi-blue border-navi-blue/40' : 'border-white/15 text-white/30 hover:border-white/30 hover:text-white/55'
                    }`}>
                    {d}
                  </button>
                ))}
            ...
            <div className="flex items-center gap-1.5 mt-1 flex-wrap">
              {!showAiDatePicker ? (
                <>
                  ...
                  <button
                    onClick={e => { e.stopPropagation(); updateCycle(area, cycle.id, { triggerLabel: aiResult.suggestedDate }); setAiResult(null); setShowAiDatePicker(false) }}
                  ...
                  >Change</button>
                  <button
                    onClick={e => { e.stopPropagation(); setAiResult(null); setShowAiDatePicker(false) }}
                ...
                <>
                  ...
                  />
                  <button
                    onClick={e => { e.stopPropagation(); if (aiCustomDate) { updateCycle(area, cycle.id, { triggerLabel: aiCustomDate }); setAiResult(null); setShowAiDatePicker(false) } }}
                    className="text-[10px] px-1.5 py-0.5 rounded bg-green-500/15 text-green-400 border border-green-500/20 hover:bg-green-500/25 transition-colors"
                  >✓ Apply</button>
                  <button
                    onClick={e => { e.stopPropagation(); setShowAiDatePicker(false) }}
                    className="text-[10px] text-white/25 hover:text-white/55 transition-colors"
                  >✗</button>
                </>
              )}
